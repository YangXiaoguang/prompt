# 提示词索引Prompt Index

## 01 分析 Analysis :mag:
- [AI 文章深度分析](../prompts/01-analysis/analysis-ai-article-deep-analysis-v1.md)

## 02 写作 Writing :memo:
- [专业文章写作增强](../prompts/02-writing/writing-professional-article-enhancement-v1.md)

## 03 研究 Research :microscope:
- [AI 科技文献/论文研究提示词 通用片](../prompts/03-research/research-ai-tech-v1.md)

- [AI 科技文献/论文研究提示词 高级版](../prompts/03-research/research-ai-tech-advanced.md) :star:
> 高级版（适合你要“高质量深度研究稿”时使用）

- [AI 技术趋势研究](../prompts/03-research/research-ai-tech-trend-v1.md)

## 04 产品 Product :gift:
- [企业级 PRD 生成](../prompts/04-product/product-enterprise-prd-generation-v1.md)
- [企业级 需求规格说明书SRS 生成](../prompts/04-product/product-enterpris-srs-generation-v1.md)
## 05 智能体 Agent :robot:
- [通用 Agent 系统指令模板](../prompts/05-agent/agent-general-system-prompt-v1.md)

## 06 代码 coding :computer:
- [vibe coding 代码安全审计 Prompt](../prompts/06-coding/coding-security-auditor-prompt-v1.md)
> 这个强大的提示功能可以将LLM变成你的生产环境安全审查员。粘贴你的代码库（或关键文件），它将审核 - [AI Vibe Coding Security Playbook](../assets/images/vibe_coding_security_playbook.png)全部 24 条规则，标记所有违规项，并在发布前提供一份优先级排序的修复列表。

## 99 元提示词模板Templates :octocat:
- [元提示词](../prompts/99-templates/MetaPrompt.md)
> “元提示词模板” 能用提示生成器
> 让 LLM 根据你的业务需求，自动生成一个专业的“智能体系统指令 / Prompt / Agent Spec”
- [结构化提示词生成器](../prompts/99-templates/structured_prompt_maker-v1.md)
- [多步探索提示词构建器](../prompts/99-templates/multi-stage-prompt-maker-v1.md)
- [通用深度分析模板](../prompts/99-templates/template-deep-analysis-universal-v1.md)
